...
export const clampWallChildPosition = (wall: Wall, child: Window | Door) => {
  const childSize = getBufferGeometrySize(child.geometry)
  const maxPosition = new Vector3(
    WALL_WIDTH / 2 + childSize.z / 2,
    wall.geometry.parameters.height - childSize.y / 2,
    wall.geometry.parameters.depth - childSize.x / 2 - 0.1
  )
